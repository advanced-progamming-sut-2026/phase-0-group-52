...
public class SignupMenu implements AppMenu{
    ...
    public void check(Scanner scanner) {
        ...
        else if(SignUpCommands.ENTER_MENU_REGEX.matches(line)) {handleEnterMenu(line);}
        else if(SignUpCommands.CURRENT_MENU_REGEX.matches(line)) {
            Result result = controller.showCurrentMenu();
            System.out.println(result.message());}
        else if(SignUpCommands.REGISTER_REGEX.matches(line)){handleRegister(line);}
        else if(SignUpCommands.PICK_SECURITY_QUESTION_REGEX.matches(line)){handlePickQuestion(line);}
        else invalidCommand();
    }
